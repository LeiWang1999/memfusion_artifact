...
class TIRLadderMMAScheduler4D(TIRSchedulerBase):
    ...
    def schedule_inconsistent(self, is_a_consistent=False, is_b_consistent=False):
        ...
        if not is_a_consistent and A_decode_block:
            # cache_decompress
        ...
        if not is_b_consistent and B_decode_block:
            # cache_decompress
    ...
    # schedule for inconsistent cdna card
    ...
    def schedule(self) -> tir.Schedule:
        ...
        if "ROCm" in self.config.arch.platform:
            wmma_k = 16
        ...
        if self.config.arch.platform == "cuda":
            ...
            if "CDNA" in self.config.arch.platform:
                if is_consistent:
                    ...
                    return self.schedule_inconsistent_rocm_cdna(is_a_consistent, is_b_consistent)
